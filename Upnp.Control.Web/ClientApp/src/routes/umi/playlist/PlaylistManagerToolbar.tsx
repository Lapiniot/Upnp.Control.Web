...
export function PlaylistManagerToolbar({ editMode, rootLevel: rootLevel, fetching, title, subtitle, service }:
    ...
    return <Toolbar className="px-2 py-1 bg-white border-bottom flex-nowrap">
        {editMode ? <>
            <Toolbar.Button glyph="sprites.svg#close" onClick={handlers.toggleEditMode} className={className} />
            ...
            <Toolbar.Button glyph="sprites.svg#chevron-left" onClick={handlers.navigateBack} className={className} />
        ...
        {rootLevel && largeScreen && !editMode &&
            <Toolbar.Button glyph="sprites.svg#plus" title={`Add new (${HotKeys.createNew})`} onClick={handlers.create} className={className} disabled={fetching} />}
        {rootLevel && (editMode || largeScreen) && <>
            <Toolbar.Button glyph="sprites.svg#trash" title={`Delete (${HotKeys.delete})`} onClick={handlers.deletePlaylists} className={className} disabled={hasNoSelection} />
            <Toolbar.Button glyph="sprites.svg#pen-to-square" title={`Rename (${HotKeys.rename})`} onClick={handlers.rename} className={className} disabled={!onlySelected} />
            <Toolbar.Button glyph="sprites.svg#copy" title={`Duplicate (${HotKeys.duplicate})`} onClick={handlers.copy} className={className} disabled={!onlySelected} />
        </>}
        {!rootLevel && largeScreen && !editMode && <>
        ...
        {editMode &&
            <Toolbar.Button glyph="sprites.svg#list-check" onClick={handlers.selectAll} className={className} />}
        {!editMode && !largeScreen &&
            <Toolbar.Button glyph="sprites.svg#pen" onClick={handlers.toggleEditMode} className={className} disabled={fetching} />}
    </Toolbar>
}