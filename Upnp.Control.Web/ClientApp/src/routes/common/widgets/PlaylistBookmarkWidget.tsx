...
export default function ({ device, id, title, icon, deviceName }: PlaylistBookmarkWidgetProps) {
    ...
    return <div className="hstack flex-fill min-w-0">
        <button type="button" className={`btn btn-stack mx-2 flex-shrink-0`}
        ...
        <RouteLink to={`/umi/${device}/playlists/${id}`} title={`${title} on ${deviceName}`}
            className="flex-fill min-w-0 text-decoration-none ps-0 p-2">
