...
internal partial class UpnpDiscoveryService
{
    ...
    [LoggerMessage(11, LogLevel.Information, "Device expiration updated for UDN='{udn}'")]
    private partial void LogExpirationUpdated(string udn);

    [LoggerMessage(12, LogLevel.Information, "New device discovered with UDN='{udn}'")]
    private partial void LogDeviceDiscovered(string udn);

    [LoggerMessage(13, LogLevel.Error, "Error processing SSDP reply {startline} for USN={usn}")]
    private partial void LogReplyError(Exception exception, string startline, string usn);

    [LoggerMessage(14, LogLevel.Error, "Error discovering UPnP devices and services!")]
    private partial void LogError(Exception exception);

    [LoggerMessage(15, LogLevel.Error, "Error providing new data to the observer: {observer}")]
    private partial void LogNotifyError(Exception exception, IObserver<UpnpDiscoveryEvent> observer);

    [LoggerMessage(16, LogLevel.Error, "Error sending completion notification to the observer: {observer}")]
    private partial void LogNotifyCompleteError(Exception exception, IObserver<UpnpDiscoveryEvent> observer);

    [LoggerMessage(17, LogLevel.Trace, "{message}")]
