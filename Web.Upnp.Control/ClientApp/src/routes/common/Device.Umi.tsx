...
function Menu({ dataContext: d, device }: HTMLAttributes<HTMLDivElement> & DataFetchProps<BrowseFetchResult> & { device?: string }) {
    return <>
        <a href="#" className="dropdown-toggle dropdown-toggle-split px-1" data-toggle="dropdown" aria-expanded="false">
            <span className="visually-hidden">Toggle Dropdown</span>
        </a>
        <ul className="dropdown-menu" data-device={device}>
            {d?.source.items.map(i => <li>
                <a className="dropdown-item" href="#" data-play-url={i.res?.url + "#play"} onClick={playUrlHandler}>{i.title}</a>
            </li>)}
        </ul>
    </>
}
